    ...
    generarCapitulo(ctx) {
        ...
        if (cats.length) {
            ...
            if (resumenSocio.length) {
                h += this._p(`La tabla anterior describe el perfil de los ${datos.length} participantes del estudio; esta caracterización delimita el alcance de la generalización de los resultados. De manera específica, ${resumenSocio.join('; ')}. Las frecuencias (f) indican el número de casos por categoría y los porcentajes su proporción respecto del total, de modo que las categorías con mayor representación definen la composición de la muestra y deben considerarse al valorar la generalización de los hallazgos.`);
            }
        }
        // ---- Niveles ----
        if (typeof calcularNivelesDeValores === 'function') {
            [[var1, et1], [var2, et2]].forEach(([col, et]) => {
                ...
                const _ord = [...r.niveles].sort((a, b) => b.f - a.f);
                const _may = _ord[0], _men = _ord[_ord.length - 1], _med = _ord.length > 2 ? _ord[1] : null;
                h += this._p(`En la tabla precedente se observa que ${_may.f} participantes (${_may.pct.toFixed(1)} %) del total de la muestra se ubicaron en el nivel ${_may.nivel.toLowerCase()} de ${et}, proporción que constituye la mayor de las tres categorías${_may.pct > 50 ? ' y supera la mitad de la muestra' : ''}, mientras que el menor porcentaje correspondió al nivel ${_men.nivel.toLowerCase()} (${_men.f} casos; ${_men.pct.toFixed(1)} %)${_med ? `; el nivel ${_med.nivel.toLowerCase()}, por su parte, concentró ${_med.f} casos (${_med.pct.toFixed(1)} %)` : ''}. Estos resultados indican que el nivel ${_may.nivel.toLowerCase()} de ${et} alcanzó una frecuencia superior a la de los demás niveles, lo que describe la tendencia predominante de la muestra en esta variable. Los puntos de corte corresponden a los terciles empíricos de la distribución (percentiles 33.3 y 66.7), criterio empleado cuando el instrumento no dispone de baremos normativos propios.`);
            });
        }
        // ---- Descriptivos ----
        ...
        if (d1 && d2) {
            ...
            h += this._p('La media constituye la medida de tendencia central que representa el punto de equilibrio de la distribución, motivo por el cual resulta sensible a la presencia de valores extremos, mientras que la desviación estándar cuantifica la dispersión promedio de las puntuaciones respecto de dicho valor. Bajo el supuesto de normalidad, aproximadamente el 68 % de las observaciones se sitúa a ±1 DE de la media y cerca del 95 % a ±2 DE. La asimetría describe el grado de concentración de las puntuaciones en una de las colas de la distribución (los valores positivos indican una cola derecha prolongada que desplaza la media por encima de la mediana), en tanto que la curtosis expresa la propensión de la distribución a producir valores extremos. Cabe precisar que el resumen «media ± DE» resulta adecuado únicamente cuando la distribución se aproxima a la normalidad; ante desviaciones marcadas de dicha forma, estos estadísticos pueden conducir a interpretaciones erróneas, razón por la cual el supuesto de normalidad se verifica formalmente antes de seleccionar las técnicas de análisis.');
        ...
        const expHist = (et, d, nn) => this._p(`La figura anterior muestra el histograma de ${et}: cada barra representa cuántos participantes obtuvieron puntajes dentro de ese intervalo, y la curva superpuesta corresponde a la distribución normal teórica con la media y desviación estándar de la muestra. Cuanto más se ajustan las barras a la curva, más plausible es la normalidad. En este caso, la asimetría de ${Number.isFinite(d?.asimetria) ? d.asimetria.toFixed(2) : '—'} ${Number.isFinite(d?.asimetria) ? (Math.abs(d.asimetria) < 0.5 ? 'sugiere una distribución esencialmente simétrica' : d.asimetria > 0 ? 'indica una cola derecha (concentración de puntajes bajos con algunos valores altos)' : 'indica una cola izquierda (concentración de puntajes altos con algunos valores bajos)') : ''} y la curtosis de ${Number.isFinite(d?.curtosis) ? d.curtosis.toFixed(2) : '—'} describe cuán apuntada o achatada es la distribución respecto de la normal. Estos indicadores resultan congruentes con la prueba de ${nn.prueba} (${nn.prueba.includes('Shapiro') ? 'W' : 'D'} = ${nn.estadistico.toFixed(3)}, p ${fp(nn.pValor)}), según la cual ${nn.normal ? 'no se rechazó' : 'se rechazó'} el supuesto de normalidad.`);
        const expQQ = (et, nn) => this._p(`El gráfico Q-Q de ${et} compara los cuantiles observados de la muestra con los que se esperarían bajo una distribución normal perfecta: si los puntos se alinean sobre la diagonal, los datos se comportan como normales; desviaciones sistemáticas en los extremos revelan colas más pesadas o livianas de lo esperado. El patrón observado en el gráfico resulta congruente con el resultado de la prueba de ${nn.prueba} (p ${fp(nn.pValor)}), que ${nn.normal ? 'no rechazó' : 'rechazó'} la hipótesis de normalidad, y esta decisión es la que determina el coeficiente de correlación apropiado.`);
        h += this._figura('histVariable1', `Distribución de ${et1} con curva normal teórica superpuesta`, `Prueba de ${n1.prueba}: ${n1.prueba.includes('Shapiro') ? 'W' : 'D'} = ${n1.estadistico.toFixed(3)}, p ${fp(n1.pValor)}.`);
        h += expHist(et1, d1, n1);
        h += this._figura('qqVariable1', `Gráfico Q-Q de ${et1}`, null);
        h += expQQ(et1, n1);
        h += this._figura('histVariable2', `Distribución de ${et2} con curva normal teórica superpuesta`, `Prueba de ${n2.prueba}: ${n2.prueba.includes('Shapiro') ? 'W' : 'D'} = ${n2.estadistico.toFixed(3)}, p ${fp(n2.pValor)}.`);
        h += expHist(et2, d2, n2);
        h += this._figura('qqVariable2', `Gráfico Q-Q de ${et2}`, null);
        h += expQQ(et2, n2);
        h += this._p(`Cabe señalar que la impresión visual del histograma puede no coincidir con la decisión de la prueba estadística sin que ello constituya una inconsistencia. Con muestras de mayor tamaño (n = ${resultado.n}), las pruebas de normalidad incrementan su potencia y detectan desviaciones sutiles —especialmente en la curtosis y en las colas— que no resultan apreciables mediante el examen gráfico. Una distribución con curtosis positiva eleva las barras centrales hasta aproximarlas al máximo de la curva teórica y genera la apariencia de un ajuste adecuado, mientras que la prueba la rechaza precisamente por ese exceso de concentración central; a la inversa, la variabilidad propia del muestreo puede sugerir desajustes locales sin que la forma global se aparte de la normalidad. Por ello, la decisión reportada se fundamenta en el contraste formal (${n1.prueba}); el gráfico Q-Q, más sensible al comportamiento de las colas, constituye la herramienta visual más adecuada para corroborarla.`);
